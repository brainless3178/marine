          ...
          {categories.map((cat, i) => {
            ...
            return (
              <TiltCard key={cat.id} className={`group card p-7 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-[30px]'}`} style={{ transitionDelay: `${i * 80}ms`, transitionProperty: 'opacity, transform', transitionDuration: '0.5s' }}>
                <div className="mb-5 flex h-14 w-14 items-center justify-center rounded-xl bg-[var(--teal-soft)] text-[var(--accent-primary)]"><Icon size={28} /></div>
                ...
                <span className="inline-block font-mono text-xs px-3 py-1.5 border text-[var(--accent-primary-hover)] border-[var(--accent-primary)]/20 bg-[var(--teal-soft)] rounded-full">
