          ...
          {categories.map((cat, i) => {
            ...
            return (
              <TiltCard key={cat.id} className={`group card p-7 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-[30px]'}`} style={{ transitionDelay: `${i * 80}ms`, transitionProperty: 'opacity, transform', transitionDuration: '0.5s' }}>
                ...
                <span className={`inline-block font-mono text-xs px-3 py-1.5 border ${isGold ? 'text-[var(--text-secondary)] border-[var(--accent-gold)]/20 bg-[var(--accent-gold)]/[0.06]' : 'text-[var(--accent-primary-hover)] border-[var(--accent-primary)]/15 bg-[var(--accent-primary)]/[0.10]'} rounded-full`}>
