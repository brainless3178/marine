      ...
      <div className="w-full max-w-md">
        ...
        <div className="mb-8 text-center">
          <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-[var(--accent-gold)] shadow-[0_12px_40px_rgba(232,170,36,0.3)]">
            <Anchor size={28} className="text-[#061522]" />
          </div>
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 p-8 backdrop-blur-xl shadow-2xl">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {/* Error */}
            {error && (
              <div className="flex items-center gap-2 rounded-lg bg-red-500/10 border border-red-500/20 px-3 py-2.5 text-xs font-semibold text-red-400">
                <AlertCircle size={14} />
                {error}
              </div>
            )}
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-[var(--accent-gold)] to-[var(--gold-light)] py-3.5 text-sm font-extrabold text-[#061522] transition-all hover:shadow-[0_8px_30px_rgba(232,170,36,0.3)] hover:-translate-y-0.5 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:translate-y-0"
            >
              {loading ? (
                <div className="h-5 w-5 rounded-full border-2 border-[#061522] border-t-transparent animate-spin" />
              ) : (
                <>
                  Sign In
                  <ArrowRight size={16} />
