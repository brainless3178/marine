    ...
    <>
      <div className="space-y-5">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <button onClick={() => fileInputEl?.click()} disabled={uploading} className="inline-flex items-center gap-2 rounded-xl bg-[var(--accent-gold)] px-4 py-2.5 text-xs font-extrabold text-[#061522] transition-all hover:bg-[var(--gold-light)] hover:-translate-y-0.5 disabled:opacity-50">
        ...
        <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-4">
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <div className="flex items-center gap-1 rounded-xl border border-[var(--border)] bg-[var(--surface-soft)] p-1">
              <button onClick={() => setViewMode('grid')} className={`flex h-8 w-8 items-center justify-center rounded-lg transition-colors ${viewMode === 'grid' ? 'bg-[var(--accent-gold)] text-[#061522]' : 'text-[var(--text-muted)] hover:text-[var(--text-primary)]'}`}><Grid3X3 size={14} /></button>
              <button onClick={() => setViewMode('list')} className={`flex h-8 w-8 items-center justify-center rounded-lg transition-colors ${viewMode === 'list' ? 'bg-[var(--accent-gold)] text-[#061522]' : 'text-[var(--text-muted)] hover:text-[var(--text-primary)]'}`}><List size={14} /></button>
            </div>
          </div>
          {(filterLabel || search) && (
        ...
        {selectedIds.size > 0 && (
          <div className="flex items-center gap-3 rounded-xl border border-[var(--accent-gold)]/30 bg-[var(--gold-muted)] px-4 py-3">
            ...
            <div className="flex flex-wrap gap-2 ml-auto">
              <button onClick={handleBulkDelete} className="rounded-lg bg-red-50 border border-red-200 px-3 py-1.5 text-[0.625rem] font-bold text-[var(--danger)] hover:bg-red-100"><Trash2 size={10} className="inline mr-1" />Delete</button>
            </div>
                      ...
                      <tr key={media.id}>
                        ...
                        <td>
                          <div className="flex items-center gap-1">
                            ...
                            <button onClick={() => setDeleteTarget(media.id)} className="flex h-7 w-7 items-center justify-center rounded-lg text-[var(--text-muted)] hover:text-[var(--danger)] hover:bg-red-50 transition-colors"><Trash2 size={12} /></button>
                          </div>
                        </td>
                      </tr>
                    )
            ...
            <div className="relative w-full max-w-2xl mx-4 rounded-2xl border border-[var(--border)] bg-[var(--surface)] shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="p-5 space-y-4">
                ...
                <div className="flex items-center gap-2 pt-2 border-t border-[var(--border)]">
                  ...
                  <button onClick={() => setDeleteTarget(previewImage.id)} className="ml-auto inline-flex items-center gap-1.5 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-xs font-bold text-[var(--danger)] hover:bg-red-100 transition-colors"><Trash2 size={12} /> Delete</button>
                </div>
              </div>
            </div>
          </div>
